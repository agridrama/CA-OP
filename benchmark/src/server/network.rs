...
impl Network {
    ...
    async fn handle_incoming_connection(
        ...
        let new_connection = match registration_message {
            ...
            Some(Ok(RegistrationMessage::ClientRegister(client_id))) => {
                info!("Identified connection from client {client_id}");
                let underlying_stream = registration_connection.into_inner().into_inner();
    ...
    pub fn connected_client_ids(&self) -> Vec<ClientId> {
        let mut ids: Vec<ClientId> = self.client_connections.keys().copied().collect();
        ids.sort_unstable();
        ids
    }

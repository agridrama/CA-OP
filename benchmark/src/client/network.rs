...
use log::*;
use omnipaxos_kv::common::{kv::{ClientId, NodeId}, messages::*, utils::*};
...
impl Network {
    pub async fn new(client_id: ClientId, servers: Vec<(NodeId, String)>, batch_size: usize) -> Self {
        ...
        network.initialize_connections(client_id, servers).await;
        network
    }

    async fn initialize_connections(&mut self, client_id: ClientId, servers: Vec<(NodeId, String)>) {
        ...
        for (server_id, server_addr_str) in &servers {
            let server_address = server_addr_str
                ...
                .next()
                .unwrap();
            let task = tokio::spawn(Self::get_server_connection(client_id, *server_id, server_address));
            ...
            match TcpStream::connect(server_address).await {
                Ok(stream) => {
                    ...
                    registration_connection
                        .send(RegistrationMessage::ClientRegister(client_id))
                        .await
